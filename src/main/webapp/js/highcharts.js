  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      for (let i = 1; i < jsonArray.length; ++i) {
        jsonArray[i][0] =
          jsonArray[i][0].substr(-4) +
          "-" +
          jsonArray[i][0].substr(0, 5) +
          "T00:00:00.000Z";
        var d = new Date(jsonArray[i][0]);
        jsonArray[i][0] = Date.parse(d);
      }
      ...
      var stockNames = [];
      for (let i = 1; i < jsonArray[0].length; ++i) {
        stockNames[i - 1] = jsonArray[0][i];
      }

      // Extract the columns from the array to create separate arrays
      var columns = new Array();
      jsonArray.shift();
      for (let i = 0; i < jsonArray[0].length; ++i) {
        columns.push(
          jsonArray.map(function (value, index) {
            return value[i];
          })
        );
      }

      // Pair the dates with associated stock value
      // [[date, value], [date, value], [date, value], ...]
      var stockArrays = [];
      for (let i = 0; i < stockNames.length; ++i) {
        var a = new Array(columns[0]);
        a.push(columns[i + 1]);
        stockArrays[i] = zip(a);
      }
...
function drawMainChart() {
  Highcharts.stockChart("container-highchart", {
    rangeSelector: {
      selected: 1,
    },
